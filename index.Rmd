...
- Dados utilisados: `english` (pacote `languageR`)
...
- Dados no **eixo y** comparados: um único eixo y com todos os painéis alinhados horizontalmente
...
Fonte: <https://royal-statistical-society.github.io/datavisguide/>

```{r, echo=FALSE, message=FALSE, out.width="90%", cache=TRUE, fig.height=4, fig.width = 8, message = FALSE, dpi=500, fig.align='center'}
...
- Uma figura quadrada evita viés visual na escolha dos eixos y e x

- Uma figura quadrada deve ser considerada se os dois eixos têm algo em comum:

   + pré x pós

   + dados observados x valores previstos pelo modelo
...
- O eixo deve começar em 0 (a menos que haja um bom motivo para escolher outros intervalos)
